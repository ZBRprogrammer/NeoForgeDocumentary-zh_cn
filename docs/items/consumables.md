...
**消耗品**(`Consumables`)是可在一段时间内使用并在过程中被"消耗"的[物品](`item`)。Minecraft中任何可食用或饮用的物品都属于消耗品。
...
- `hasConsumeParticles` - 为`true`时每4刻及完全消耗时生成物品[粒子](`particles`)。默认为`true`。
- `onConsume` - 添加[`ConsumeEffect`](`consumeeffect`)，在`Item#finishUsingItem`完全消耗物品时应用。

原版在`Consumables`类中提供部分消耗品，如[食物](`food`)的`#defaultFood`和[药水](`potions`)与牛奶桶的`#defaultDrink`。
...
首先使用[可扩展枚举](`extensibleenum`)系统创建新`ItemUseAnimation`：
...
消耗品和消耗后应用的效果虽有用，但有时效果属性需作为其他[数据组件](`datacomponents`)外部可用。例如猫和狼也吃[食物](`food`)并查询其营养值，或药水物品为渲染查询颜色。此时数据组件实现`ConsumableListener`提供消耗逻辑。

`ConsumableListener`仅有一个方法：`#onConsume`（参数：当前世界、消耗实体、被消耗物品、物品上的`Consumable`实例）。`onConsume`在物品完全消耗时于`Item#finishUsingItem`中调用。

添加自定义`ConsumableListener`只需[注册新数据组件](`datacompreg`)并实现`ConsumableListener`。
...
- `saturationModifier` - 计算[饱和度](`hunger`)恢复的修饰符（公式：`min(2 * nutrition * saturationModifier, playerNutrition)`）。0.5时有效饱和度等于营养值。
